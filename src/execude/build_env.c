...
int create_pipes(t_execute *execute)
{
    ...
    execute->i = 0;
    while (execute->i < execute->num_pipes)
    {
...
char *resolve_command_path(char *command, char **envp)
{
    if(command[0] == '/' || command[0] == '.')
        return ft_strdup(command);
    char *path = find_env_value(env_list, "PATH");
...
void execute_command(t_execute *execute)
{
    ...
    execute->cmd_path = resolve_command_path(execute->commands[execute->i][0], execute->envp);
...
void close_pipes_and_wait(int *pipe_fds, int num_pipes, pid_t *pids, size_t num_cmds)
{
    int i = 0;
    while (i < 2 * num_pipes)
    {
...
void start_execution(t_token *tokens, size_t token_count, t_env *env_list)
{
    ...
    if (fill_commands(tokens, token_count, execute.num_cmds) || !execute.cmd_args
        || create_pipes(&execute) || fork_and_execute(&execute))
    {
        free_execute(&execute);
        return;
    }
    close_pipes_and_wait(execute.pipefds, execute.num_pipes, execute.pids, execute.num_cmds);
    free_execute(&execute);
}
